...
require('../admin/fpdf184/fpdf.php');
...
class CustomPDF extends FPDF
{
    ...
    protected $widths;
    protected $aligns;
    protected $tblHdrWidths; // solo se usa cuando se necesita un salto de página.
    protected $tblHdrAligns; // solo se usa cuando se necesita un salto de página.
    ...
    public function __construct($orientation = 'P', $pageHeaderRepeat = false, $reportTitle = 'Reporte', $fromDate = '', $toDate = '', $isFooterRequired = true)
    {
    ...
    {
        $this->resetTableHeaders();
        // si la leyenda y el encabezado (una o dos alturas) y la primera fila se pueden mostrar
    ...
    public function AddTableHeader(
        ...
        // si el encabezado y la primera fila se pueden mostrar
        ...
        // si ninguna columna necesita colspan
    ...
    public function AddRow($data, $drawBorder = true, $fillRow = false, $isHeaderRow = false)
    {
        ...
        for ($i = 0; $i < count($data); $i++) {
            ...
            $x = $this->GetX();
            $y = $this->GetY();
            // Dibujar el borde y el área de relleno
            if (!$isHeaderRow) {
    ...
    public function Header()
    {
        if ($this->pageHeaderRepeat || (!$this->pageHeaderRepeat && !$this->pageHeaderAdded)) {
            // Logo
            //$this->Image('assets/dist/img/pipoicon.png', $this->GetX(), $this->GetY(), 30);
            // Times bold 13
            $this->SetFont('Times', '', 18);
            $this->SetTextColor(191, 0, 0);
            // Título
            $this->Cell(0, 7, 'PACIENTES', 0, 0, 'C');
            $this->Ln();
            ...
            if ($this->fromDate != '' || $this->toDate != '') {
                if ($this->fromDate != '' && $this->toDate != '') {
                    $this->Cell(0, 8, 'Desde ' . $this->fromDate . '  hasta  ' . $this->toDate, 0, 0, 'C');
                } else if ($this->fromDate != '') {
                    $this->Cell(0, 7,  $this->fromDate, 0, 0, 'C');
                }
            }
            // Salto de línea.
            $this->Ln(2);    // La altura del salto se puede especificar
    ...
    {
        // Establecer el arreglo de anchos de columna
        $this->widths = $w;
    }

    public function SetAligns($a)
    {
        // Establecer el arreglo de alineaciones de columna para encabezados de tabla
        $this->aligns = $a;
    }
